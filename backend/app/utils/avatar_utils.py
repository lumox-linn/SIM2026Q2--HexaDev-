...
def get_avatar_url(role: str, profile_picture: str = None) -> str:
    ...
    Uses request.host_url so the URL works on any domain:
      localhost → http://localhost:5000/static/uploads/abc.jpg
      Railway   → https://myapp.railway.app/static/uploads/abc.jpg

    This is exactly what Yimin described:
    backend changes the random local filename into a URL anyone can access.
    """
    base_url = request.host_url.rstrip('/')

    if profile_picture:
        # User has uploaded a custom picture
        return f"{base_url}/static/uploads/{profile_picture}"
    else:
        # Fall back to default role avatar
